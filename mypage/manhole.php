...
if (isset($_SESSION['id']) && ($_SESSION['time'] + 3600 > time())) {
    ...
    // ログインしているユーザーのIDに基づいてテーブルからデータを取得
...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $sql = 'INSERT INTO manhole_image(image_name, image_type, image_content, image_size, user_id, prefecture, comment, created_at)
                VALUES (:image_name, :image_type, :image_content, :image_size, :user_id, :prefecture, :comment, now())';
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(':image_name', $name, PDO::PARAM_STR);
        $stmt->bindValue(':image_type', $type, PDO::PARAM_STR);
        $stmt->bindValue(':image_content', $content, PDO::PARAM_STR);
        $stmt->bindValue(':image_size', $size, PDO::PARAM_INT);
        $stmt->bindValue(':prefecture', '', PDO::PARAM_STR); // 県名は初期状態で空
        $stmt->bindValue(':comment', '', PDO::PARAM_STR); // コメントは初期状態で空
                ...
                <?php for($i = 0; $i < count($images); $i++): ?>
                    <li class="media mt-5">
                        ...
                        <div class="media-body">
                                <!-- 編集ボタン -->
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#editModal<?= $images[$i]['image_id']; ?>">
                                    編集
                                </button>
                            </div>
                            <div class="media-body">
                                <!-- 削除ボタン -->
                                <a href="javascript:void(0);" onclick="var ok = confirm('削除しますか？'); if (ok) location.href='delete_m.php?id=<?= $images[$i]['image_id']; ?>'">
                    ...
                    <!-- 編集フォームのモーダル -->
                    <div class="modal fade" id="editModal<?= $images[$i]['image_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editModalLabel">メモ追加</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <form method="post" action="update_manhole.php">
                                        <input type="hidden" name="image_id" value="<?= $images[$i]['image_id']; ?>">
                                        <div class="form-group">
                                            <label>県名</label>
                                            <input type="text" name="prefecture" class="form-control" value="<?= $images[$i]['prefecture']; ?>">
                                        </div>
                                        <div class="form-group">
                                            <label>コメント</label>
                                            <textarea name="comment" class="form-control"><?= $images[$i]['comment']; ?></textarea>
                                        </div>
